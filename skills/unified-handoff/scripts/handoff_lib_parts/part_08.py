...
def validate_handoff(path: Path, project_root: Path, config: Mapping[str, Any]) -> ValidationResult:
    ...
    secret_findings, secret_warnings = scan_secrets(
        content, [str(item) for item in config.get("custom_secret_patterns", [])]
    )
    ...
    if mode in ("standard", "full") and not meaningful_section(
        "User Requirements and Constraints", constraints, mode
    )[0]:
        score -= 2
...
def finalize_handoff(path: Path, project_root: Path, config: Mapping[str, Any]) -> Tuple[ValidationResult, Optional[Path]]:
...
def list_handoffs(project_root: Path, config: Mapping[str, Any], include_legacy: bool = False) -> List[Dict[str, Any]]:
    paths = resolve_paths(project_root, config)
